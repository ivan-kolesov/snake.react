...
class GameScreen extends React.Component {
    ...
    for (let i = 0; i < sourceSnake.length; i++) {
      if (i === 0) {
        if (direction === DIRECTION_RIGHT) {
          if (
            snake[i].x + boardConstants.segmentRate >=
  ...
  makeFood = snake => {
    ...
    if (
      !_.some(snake, segment => _.isEqual(food, { x: segment.x, y: segment.y }))
    ) {
      return food;
    }
  ...
  eatFood = () => snake => {
    const { score, intervalRate, setScore, setFood, setIntervalRate } =
      this.props;
          ...
          <View style={styles.container}>
            ...
            <View style={styles.boardContainer}>
              <Board
                ...
                setDirectionPan={setDirectionPan}
                snake={snake}
                food={food}
              />
            </View>
